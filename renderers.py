...
class Raymarcher(nn.Module):
    ...
    def forward(self,
        ...
        return None, rgb, final_depth, final_depth
...
class AdaptiveVolumeRenderer(nn.Module):
    def __init__(self, num_feature_channels, raymarch_steps, epsilon, n_coarse, white_back, separate):
        super().__init__()
        ...
        print(f'separation status: {separate}')
    ...
    def forward(
        ...
        if debug:
            ...
            sigma = sigma_rad[...,3].view(SB, num_rays, 64) # (SB, num_rays, n_coarse, 1)
        ...
        # Coarse image
        output_coarse = phi(world_coords[-1].reshape(SB,-1,3), viewdirs = rds.reshape(SB,-1,3), coarse = self.separate, return_features = False)
        ...
        random_ind = 0
        print(depth_map[0][random_ind])

        return rgb_coarse, rgb, depth_coarse, depth_map

    @classmethod
    def from_conf(cls, conf, white_back=False, separate = False):
        return cls(
            num_feature_channels=conf.get_int("num_feature_channels", 512),
            raymarch_steps = conf.get_int("raymarch_steps", 10),
            ...
            white_back=conf.get_float("white_back", white_back),
            separate=conf.get_float("separate", separate)
        )