...
#             rds = None
    ...
    #                           (model_output[0].min().detach().cpu().numpy(), model_output[0].max().detach().cpu().numpy()))
            ...
            if not step % val_steps: # visualization
                ...
                with torch.no_grad():
                    val_output = model(to_gpu(vval_input), debug=True)
                    psnr, ssim = get_metrics(val_output, val_gt)
                    loss = loss_fn(val_output, val_gt, loss_params)
                    if not no_visualization:
                        plot_output_ground_truth(val_output, val_gt, (sl,sl,3))
                model.train()
                if net.stop_encoder_grad:
                    net.encoder.eval()
                    
                print(f"Validation: Epoch {e} Step {step}: loss = {float(loss):.5f}, psnr = {float(psnr):.5f}, ssim = {float(ssim):.5f}")
...
if __name__ == "__main__":
    ...
    p.add_argument('--epochs', type=int, default=30, required=False,
    ...
    p.add_argument('--separate', action='store_true', required=False,
                   help='Whether to separate raymarcher and AVR color generator')
