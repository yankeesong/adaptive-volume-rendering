...
# Plotting functions
def plot_output_ground_truth(vis_output, vis_gt, resolution, fine=True):
    rgbs_coarse, rgbs_fine, _, vis_depth = vis_output
    vis_img = rgbs_fine if fine else rgbs_coarse
    ...
    fig = axes[0, 2].imshow(depth.cpu().view(*resolution[:2]).detach().numpy(), cmap='Greys')
    axes[0, 2].set_title("Depth")
        
    for j in range(3):
        axes[0,j].set_axis_off()
    plt.colorbar(fig, ax=axes[0,2])
    plt.show()
...
def generate_video(model_input, num_frames, radius, net, model, fine=True):
    # Encode the model
    ground_truth = model_input["images"] # (SB, NV, H*W, 3), values are between 0 and 1

    _, NV, sl2, _ = ground_truth.shape
    sl = int(np.sqrt(sl2))

    src_images = to_gpu(ground_truth[0:1,0:1,...].reshape(1,-1,sl,sl,3).permute(0,1,4,2,3))    # (1,1,sl,sl,3)
    poses = to_gpu(model_input['cam2world'][0:1,0:1,...])     # (1,1,4,4)
    focal = to_gpu(model_input['focal'][0,0]) # scalar
    c = to_gpu(model_input['c'][0,0,:])   
    intrinsics = to_gpu(model_input['intrinsics'][0:1,0,...])     # (1,3,3)
    net.encode(src_images, poses, focal, c)
    
    # Generate a round of camera views
    angles = torch.linspace(0,2*np.pi*(num_frames-1)/num_frames,num_frames) + np.pi/num_frames
    cam2world = []
    z_height = 0.4
    rradius = np.sqrt(radius*radius-z_height*z_height)
    for i in range(num_frames):
        angle = angles[i]
        tx = rradius * np.sin(angle)
        ty = rradius * np.cos(angle)
        tz = z_height
        R = get_R(tx,ty,tz)
        c2w = np.zeros((4,4))
        c2w[:3,:3] = R
    ...
    with torch.no_grad():
        ...
        for i in range(len(cam2world)):
            print(i)
            model_input = {'cam2world': cam2world[i].reshape(1,1,4,4).expand(1,sl2,-1,-1), 'intrinsics': intrinsics, 
                        'focal':focal,'c':c,'x_pix': x_pix.reshape(1,-1,2)}
            model_output = model(to_gpu(model_input))
            img_coarse, img_fine, _, _ = model_output
            img = img_fine if fine else img_coarse
