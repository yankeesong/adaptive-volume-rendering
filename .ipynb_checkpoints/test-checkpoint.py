...
def test_approximate(
    ...
    for test_input in test_dataloader:
        ...
        src_idx = torch.tensor([0]).unsqueeze(1).expand(SB, NS)    # The first image is the specific observation we want.
...
#         total_lpips = 0
        ...
        with torch.no_grad():
            nv = torch.randint(1,NV,(1,))     # Then we don't need to sample the first image anymore.
            ttest_input = {}
            ...
            psnr, ssim = get_metrics(test_output, test_gt, fine=(loss_params[0]!='coarse'))
            loss = loss_fn(test_output, test_gt, loss_params)
            rgbs_coarse, rgbs_fine, _, _ = test_output
            rgbs = rgbs_fine if loss_params[0] != 'coarse' else rgbs_coarse
